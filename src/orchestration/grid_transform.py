...
def main(argv: list[str]) -> int:
    ...
    if not argv or argv in (["-h"], ["--help"]):
        print(
            "Usage: scripts/inversion_si.py grid-transform <scripts/run/*.py> [args...]"
        )
        print(
            "Example: scripts/inversion_si.py grid-transform "
            "run_create_speaker_grid.py --source vtln --speaker 1640_P7_S2_F0829"
        )
