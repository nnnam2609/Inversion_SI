...
import render_analyze_asd2_epoch211_selected_experiment as video_core  # noqa: E402
import run_asd2_epoch211_selected_experiment as experiment  # noqa: E402
...
def frame_rmse_mm(predicted: np.ndarray, target: np.ndarray, indices: tuple[int, ...]) -> np.ndarray:
    difference = predicted[:, list(indices)].astype(np.float64) - target[:, list(indices)].astype(np.float64)
    return np.sqrt(np.mean(difference * difference, axis=(1, 2, 3))) * MM_PER_PIXEL


def class_frame_rmse_mm(predicted: np.ndarray, target: np.ndarray) -> np.ndarray:
...
def static_class_rmse_mm(predicted: np.ndarray, target: np.ndarray) -> np.ndarray:
    difference = predicted.astype(np.float64) - target.astype(np.float64)
    return np.sqrt(np.mean(difference * difference, axis=(1, 2))) * MM_PER_PIXEL
...
def build_report(
    ...
    lines.extend(
        [
            ...
            "```bash",
            f"cd {REPO_ROOT}",
            f"../inversion/.venv/bin/python -u scripts/investigate_asd2_same_vowel_u_grid.py --canonical-root {CANONICAL_ROOT} --output-root {output_root} --bootstrap-replicates 10000",
            "```",
            "",
