...
EXPECTED_SPLIT_SESSIONS = {
    "train_sequences": 85,
    "valid_sequences": 27,
    "test_sequences": 27,
}
EXPECTED_SPLIT_CHUNKS = {
    "train_sequences": 7504,
    "valid_sequences": 2478,
    "test_sequences": 2473,
}
SCHEMA_VERSION = 1
...
def iter_sessions(config: dict[str, Any]) -> list[dict[str, str]]:
    ...
    if split_counts != EXPECTED_SPLIT_SESSIONS or len(jobs) != 139:
        raise ValueError(
            f"Expected 139 sessions with split counts {EXPECTED_SPLIT_SESSIONS}, got {split_counts}"
        )
    return jobs
...
def summarize_validation(
    ...
    if split_sessions != EXPECTED_SPLIT_SESSIONS:
        raise AssertionError(f"Final split session counts mismatch: {split_sessions}")
    if split_chunks != EXPECTED_SPLIT_CHUNKS:
        raise AssertionError(f"Final split chunk counts mismatch: {split_chunks}")
    summary = {
        "status": "complete",
        "schema_version": SCHEMA_VERSION,
        "variant_name": variant_name,
...
def main() -> None:
    ...
    if selected is not None:
        ...
        if missing:
            raise ValueError(f"Selected sessions are not in the 139-session config: {sorted(missing)}")
    runtime_jobs = [
        {
            **job,
